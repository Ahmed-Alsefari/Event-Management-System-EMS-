...
public class EventController {
    ...
    // Save Event - FIXED: Properly handle ServicePackage relationship
    @PostMapping("/book")
    public String saveEvent(@ModelAttribute Event event,
                            @RequestParam(required = false) Long servicePackageId,
                            @AuthenticationPrincipal UserDetails userDetails) {

        User user = userService.findByEmail(userDetails.getUsername())
                .orElseThrow(() -> new RuntimeException("User not found"));

        // FIXED: Link the event to the logged-in user
        event.setUser(user);

        // FIXED: Properly set the ServicePackage by fetching it from the repository
        if (servicePackageId != null) {
            ServicePackage servicePackage = servicePackageRepository.findById(servicePackageId)
                    .orElseThrow(() -> new RuntimeException("Service package not found"));
            event.setServicePackage(servicePackage);
        }
        ...
        try {
            ...
            e.printStackTrace();
            return "redirect:/dashboard/book?error";
        }
    }
    ...
    public String editEventPage(@PathVariable Long id,
        ...
        // SECURITY: Check if this event belongs to the logged-in user
        if (!event.getUser().getId().equals(user.getId())) {
            return "redirect:/dashboard/profile?error=unauthorized";
        }
    ...
    // UPDATE EVENT - FIXED: Properly handle ServicePackage relationship
    ...
    public String updateEvent(@ModelAttribute Event event,
        ...
        // FIXED: Properly set the ServicePackage
        if (servicePackageId != null) {
            ServicePackage servicePackage = servicePackageRepository.findById(servicePackageId)
                    .orElseThrow(() -> new RuntimeException("Service package not found"));
            existing.setServicePackage(servicePackage);
        }
